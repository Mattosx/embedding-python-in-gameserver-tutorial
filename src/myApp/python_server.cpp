...
PythonConnection::PythonConnection( PythonServer *owner, uv_tcp_t *client, std::string welcomeString ):
		pClient_(client),
		owner_( owner ),
...
void PythonServer::onNewConnection(uv_stream_t *server, int status) {
    ...
    if (uv_accept(server, (uv_stream_t*) client) == 0) {
		connections_.push_back(new PythonConnection( this, client, welcomeString_ ));
    }
    else {
